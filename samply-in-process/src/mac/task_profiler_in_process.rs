...
use framehop::{CacheNative, FrameAddress, MayAllocateDuringUnwind};
use framehop_in_process::{InProcessUnwinder, InProcessUnwinderCache, StackCollectionOutput};
...
use super::proc_maps::{proc_cmdline, DyldInfoManager, Modification, VmSubData};
use super::sampler::{ProcessSpecificPath, TaskInit};
use super::thread_profiler::{get_thread_id, get_thread_name};
use crate::mac::thread_profiler_in_process::{StackwalkerRefInProcess, ThreadProfilerInProcess};
...
// Function pointer type for the callbacks
type DyldCallback = extern "C" fn(*const c_void, isize);

extern "C" {
    pub fn _dyld_register_func_for_add_image(func: DyldCallback);
    pub fn _dyld_register_func_for_remove_image(func: DyldCallback);
}
struct GlobalImageList {
    images: Vec<*const c_void>,
    unwinders: Vec<Weak<Mutex<InProcessUnwinder>>>,
}

unsafe impl Send for GlobalImageList {}

static GLOBAL_IMAGE_LIST: Mutex<Option<GlobalImageList>> = Mutex::new(None);

impl GlobalImageList {
    fn with_singleton<T, F>(f: F) -> T
    where
        F: FnOnce(&mut GlobalImageList) -> T,
    {
        {
            let mut locked_list = GLOBAL_IMAGE_LIST.lock().unwrap();
            if let Some(list) = &mut *locked_list {
                return f(list);
            }

            *locked_list = Some(GlobalImageList {
                images: Vec::new(),
                unwinders: Vec::new(),
            });
        }
        ...
        unsafe {
            _dyld_register_func_for_add_image(Self::add_module_func);
            _dyld_register_func_for_remove_image(Self::remove_module_func);
        }
        ...
        unreachable!("GLOBAL_IMAGE_LIST should be Some now.");
    }

    extern "C" fn add_module_func(mh: *const c_void, _slide: isize) {
        let mut locked_list = GLOBAL_IMAGE_LIST.lock().unwrap();
        if let Some(list) = &mut *locked_list {
            list.images.push(mh);
            for unwinder in &mut list.unwinders {
                if let Some (unwinder) = unwinder.upgrade() {
                    let mut unwinder = unwinder.lock().unwrap();
                    unsafe { unwinder.add_module_macho(mh) };
                }
            }
        }
    }

    extern "C" fn remove_module_func(mh: *const c_void, _slide: isize) {
        let mut locked_list = GLOBAL_IMAGE_LIST.lock().unwrap();
        if let Some(list) = &mut *locked_list {
            list.images.retain(|img| *img != mh);
            for unwinder in &mut list.unwinders {
                if let Some (unwinder) = unwinder.upgrade() {
                    let mut unwinder = unwinder.lock().unwrap();
                    unsafe { unwinder.on_before_module_removed_from_process(mh) };
                }
            }
        }
    }

    pub fn create_unwinder() -> Arc<Mutex<InProcessUnwinder>> {
        Self::with_singleton(|list| {
            let mut unwinder = InProcessUnwinder::new();
            for image in &list.images {
                unsafe { unwinder.add_module_macho(*image) };
            }
            let unwinder = Arc::new(Mutex::new(unwinder));
            list.unwinders.push(Arc::downgrade(&unwinder));
            unwinder
        })
    }
}
...
pub struct TaskProfilerInProcess {
    task: mach_port_t,
    pid: u32,
    live_threads: HashMap<thread_act_t, ThreadProfilerInProcess>,
    ...
    unwinder: Arc<Mutex<InProcessUnwinder>>,
    unwinder_cache: InProcessUnwinderCache,
...
impl TaskProfilerInProcess {
    pub fn new(
        ...
        let main_thread = ThreadProfilerInProcess::new(
            main_thread_tid,
            main_thread_handle,
            main_thread_label,
            main_thread_act,
        ...
        for thread_act in thread_act_iter {
                ...
                (get_thread_id(thread_act), get_thread_name(thread_act))
            {
                ...
                let thread = ThreadProfilerInProcess::new(
                    tid,
                    profile_thread,
                    thread_label,
                    thread_act,
        ...
        let task_profiler = TaskProfilerInProcess {
            task,
            pid,
            live_threads,
            executable_name: "this process".to_string(),
            profile_process,
            main_thread_handle,
            main_thread_label,
            ignored_errors: Vec::new(),
            unwinder: GlobalImageList::create_unwinder(),
            unwinder_cache: InProcessUnwinderCache::new(),
            path_receiver,
    ...
    fn sample_impl(
        ...
        for thread_act in thread_acts {
            ...
            let unwinder = self.unwinder.lock().unwrap();
            let stackwalker = StackwalkerRefInProcess::new(&unwinder, &mut self.unwinder_cache);
            let mut count = 0;
            let mut output = StackCollectionOutput {
                pc_buf: &mut self.pc_buf,
                sp_buf: &mut self.sp_buf,
                count: &mut count,
            };
