...
use crate::mac::task_profiler_in_process::TaskProfilerInProcess;
...
enum Tp {
    InProcess(TaskProfilerInProcess),
    Other(TaskProfiler),
}
...
impl Sampler {
    ...
    pub fn run(self) -> Result<Profile, SamplingError> {
        ...
        let root_task = TaskProfilerInProcess::new(
            root_task_init,
            timestamp_converter,
        ...
        let mut live_tasks = vec![Tp::InProcess(root_task)];
        ...
        loop {
            loop {
                ...
                if let Ok(new_task) = TaskProfiler::new(
                    ...
                    live_tasks.push(Tp::Other(new_task));
                } else {
            ...
            for task in tasks.into_iter() {
                match task {
                    Tp::InProcess(mut task) => {
                        ...
                        if still_alive {
                            live_tasks.push(Tp::InProcess(task));
                        } else {
        ...
        for task in live_tasks.into_iter() {
            let (process_sample_data, _process_recycling_data) =
                match task {
                    Tp::InProcess(task) => task.finish(&mut jit_category_manager, &mut profile),
                    Tp::Other(task) => task.finish(&mut jit_category_manager, &mut profile),
                };
            process_sample_datas.push(process_sample_data);
        }
