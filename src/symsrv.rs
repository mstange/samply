...
impl SymbolCache {
    ...
    /// `path` should be a path of the form firefox.pdb/HEX/firefox.pdb
    async fn get_pdb_impl(&self, path: &Path) -> Result<FileContents, Error> {
        assert!(path.extension() == Some(OsStr::new("pdb")));
        let mut compressed_path = path.to_owned();
        compressed_path.set_extension("pd_");
            ...
            match entry {
                NtSymbolPathEntry::Cache(cache_path) => {
                    ...
                    if let Some(file_contents) = self
                        .check_directory(cache_path, parent_cache_paths, path, &compressed_path)
                        .await?
                    {
                ...
                NtSymbolPathEntry::Chain {
                    ...
                    for url in urls {
                        if let Some(file_contents) = self
                            .check_url(url, &parent_cache_paths, path, &compressed_path)
                            .await?
                        {
                            return Ok(file_contents);
                        }
                ...
                NtSymbolPathEntry::LocalOrShare(dir_path) => {
                    ...
                    if let Some(file_contents) = self
                        .check_directory(dir_path, &persisted_cache_paths, path, &compressed_path)
                        .await?
                    {
                        return Ok(file_contents);
                    };
    ...
    async fn check_directory(
        ...
        let full_candidate_path = dir.join(path);
        let full_candidate_path_compr = dir.join(&compressed_path);

        let (rel_path, abs_path, is_compressed) =
            if self.check_file_exists(&full_candidate_path).await {
                (path, full_candidate_path, false)
            } else if self.check_file_exists(&full_candidate_path_compr).await {
                (compressed_path, full_candidate_path_compr, true)
            } else {
                return Ok(None);
            };
        ...
        let uncompressed_path = if is_compressed {
            ...
            if let Some((bottom_most_cache, mid_level_caches)) = parent_cache_paths.split_first() {
                ...
                self.copy_file_to_caches(rel_path, &abs_path, mid_level_caches)
                    .await;
                self.extract_to_file_in_cache(&mmap[..], path, bottom_most_cache)
                    .await?
            } else {
    ...
    async fn check_url(
        ...
        let full_candidate_url = url_join(url, path.components());
        let full_candidate_url_compr = url_join(url, compressed_path.components());
        let (rel_path, bytes, is_compressed) =
            match self.get_bytes_from_url(&full_candidate_url_compr).await {
                Some(bytes) => (compressed_path, bytes, true),
                None => match self.get_bytes_from_url(&full_candidate_url).await {
                    Some(bytes) => (path, bytes, false),
                    None => return Ok(None),
                },
            };
        ...
        let file_contents = if is_compressed {
            if let Some((bottom_most_cache, mid_level_caches)) = parent_cache_paths.split_first() {
                    ...
                    mid_level_caches.split_first()
                {
                    if let Ok(abs_compressed_path) = self
                        .save_file_to_cache(&bytes[..], rel_path, one_mid_level_cache)
                        .await
                    {
                        let _ = self
                            .copy_file_to_caches(
                ...
                let uncompressed_path = self
                    .extract_to_file_in_cache(&bytes[..], rel_path, bottom_most_cache)
                    .await?;
                let file = tokio::fs::File::open(&uncompressed_path).await?;
            ...
            if let Some((bottom_most_cache, mid_level_caches)) = parent_cache_paths.split_first() {
                ...
                if let Ok(abs_compressed_path) = self
                    .save_file_to_cache(&bytes[..], rel_path, bottom_most_cache)
                    .await
                {
                    let _ = self
                        .copy_file_to_caches(rel_path, &abs_compressed_path, mid_level_caches)
                        .await;
                }
            } else {
