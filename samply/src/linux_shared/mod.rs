    ...
    U: Unwinder<Module = Module<Vec<u8>>> + Default,
{
    ...
    fn process_perf_map_line(line: &str) -> Option<(u64, u64, String)> {
        let mut split = line.split_whitespace();
        let addr = split.next()?;
        let len = split.next()?;
        let symbol_name = split.collect::<Vec<_>>().join(" ");
    ...
    pub fn try_load_perf_map(&mut self, pid: i32) {
        let Ok(content) = fs::read_to_string(format!("/tmp/perf-{pid}.map")) else { return };
        ...
        for line in content.lines() {
            ...
            // HACK: prefix the symbol name with JS:~ so Gecko profiler would show it? (not
            // effective)
            let (category, js_name) = self
                .jit_category_manager
                .classify_jit_symbol(&format!("JS:~{symbol_name}"), &mut self.profile);

            process.jit_functions.insert(JitFunction {
                start_address: addr,
                end_address: addr + len,
                category,
                js_name,
            });
        }
