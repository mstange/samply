...
#[cfg(feature = "api")]
use crate::file_resolver::WholesymFileContents;
use crate::file_resolver::{CandidatePathInfo, FileResolver, WholesymFileTypes};

/// Try each debug-file candidate from `file_resolver.get_candidate_paths_for_debug_file`,
/// returning the first symbol map whose `debug_id` matches `info.debug_id`.
///
/// If `file_resolver.get_symbol_map_for_library` already has a precomputed symbol
...
pub async fn load_symbol_map_for_library_info(
    ...
    if let Some((fl, sm)) = file_resolver.get_symbol_map_for_library(info) {
    ...
    let candidates = file_resolver
        .get_candidate_paths_for_debug_file(info)
        .map_err(|e| Error::GetCandidatePathsForDebugFile(Box::new(info.clone()), e))?;

    let mut errors: Vec<Error> = Vec::new();
    for candidate in candidates {
        let result = drive_single_candidate_symbol_map(file_resolver, candidate, debug_id).await;
        match result {
...
async fn drive_single_candidate_symbol_map(
    ...
    drive_symbol_map(&mut sm, file_resolver).await;
    sm.finish()
}

/// Try each binary candidate from `file_resolver.get_candidate_paths_for_binary`,
...
pub async fn load_binary_for_library_info(
    ...
    let candidates = file_resolver
        ...
        .map_err(Error::GetCandidatePathsForBinary)?;
...
async fn drive_single_candidate_binary(
    ...
    drive_with_resolver(&mut sm, file_resolver).await;
    sm.finish()
}
