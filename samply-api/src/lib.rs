...
pub enum QueryApiJsonResult<FT: FileTypes> {
    SymbolicateResponse(symbolicate::response_json::Response<FT>),
    SourceResponse(source::response_json::Response),
    AsmResponse(asm::response_json::Response),
    Err(Error),
}

impl<FT: FileTypes> From<symbolicate::response_json::Response<FT>> for QueryApiJsonResult<FT> {
    fn from(value: symbolicate::response_json::Response<FT>) -> Self {
        QueryApiJsonResult::SymbolicateResponse(value)
    }
}

impl<FT: FileTypes> From<source::response_json::Response> for QueryApiJsonResult<FT> {
    fn from(value: source::response_json::Response) -> Self {
        QueryApiJsonResult::SourceResponse(value)
    }
}

impl<FT: FileTypes> From<asm::response_json::Response> for QueryApiJsonResult<FT> {
    fn from(value: asm::response_json::Response) -> Self {
        QueryApiJsonResult::AsmResponse(value)
    }
}

impl<FT: FileTypes> From<Error> for QueryApiJsonResult<FT> {
    fn from(value: Error) -> Self {
        QueryApiJsonResult::Err(value)
    }
}

impl<FT: FileTypes> QueryApiJsonResult<FT> {
...
impl<FT: FileTypes> Serialize for QueryApiJsonResult<FT> {
...
impl Api {
    ...
    pub fn build_query<FT: FileTypes + Send + Sync + 'static>(
        request_url: &str,
        request_json_data: &str,
    ) -> Result<Box<dyn ApiQueryState<FT> + Send>, Error>
    where
        FT::FL: Send + Sync,
    {
        match request_url {
            "/symbolicate/v5" => {
                let state = SymbolicateApiQueryState::<FT>::from_request_json(request_json_data)?;
                Ok(Box::new(state))
            }
            "/source/v1" => {
                let state = SourceApiQueryState::<FT>::from_request_json(request_json_data)?;
                Ok(Box::new(state))
            }
            "/asm/v1" => {
                let state = AsmApiQueryState::<FT>::from_request_json(request_json_data)?;
                Ok(Box::new(state))
            }
