...
pub struct SymbolicateApiQueryState<H: FileTypes> {
    state: SymbolicateState<H>,
}

struct SymbolicateContext<H: FileTypes> {
    request: request_json::Request,
    pending_libs: VecDeque<(Lib, Vec<u32>)>,
    results: HashMap<Lib, Result<LibSymbols<H>, samply_symbols::Error>>,
...
enum SymbolicateState<H: FileTypes> {
    AwaitingSymbolMap {
        ctx: SymbolicateContext<H>,
        current_lib: Lib,
    ...
    Done(Result<response_json::Response<H>, Error>),
    Poisoned,
}

impl<H: FileTypes> SymbolicateApiQueryState<H> {
    ...
    fn advance(&mut self, mut ctx: SymbolicateContext<H>) {
    ...
    fn process_lib_addresses(
        ...
        let pending_externals: VecDeque<(u32, ExternalFileAddressRef)> =
            external_addresses.into();
...
impl<H: FileTypes> ApiQueryState<H> for SymbolicateApiQueryState<H> {
...
fn record_failure<H: FileTypes>(
    ctx: &mut SymbolicateContext<H>,
    lib: &Lib,
